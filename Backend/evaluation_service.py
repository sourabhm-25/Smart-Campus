...
Architecture:
  ...
  3. Subject routing selects the right evaluation strategy before calling LLaVA.
...
def _criteria_rubric(marks: int, subject: str, question: str, answer: str) -> list[dict]:
    ...
    rubric = []
    per = math.floor(marks / marks)  # 1 per criterion
...
def _build_stepwise_prompt(question: str, correct_answer: str, rubric: list[dict], max_marks: int) -> str:
    """Math: check each step independently."""
    rubric_text = "\n".join(
...
def _build_criteria_prompt(question: str, correct_answer: str, rubric: list[dict], max_marks: int, subject: str) -> str:
    """Science/History/Geography: criteria-based with keyword and diagram awareness."""
    rubric_text = "\n".join(
        f"  Criterion {i+1} [{r['marks']} mark{'s' if r['marks'] > 1 else ''}]: {r['criterion']}"
        for i, r in enumerate(rubric)
    )
...
def _build_holistic_prompt(question: str, correct_answer: str, rubric: list[dict], max_marks: int) -> str:
    """English/Essay: holistic band-based grading."""
    rubric_text = "\n".join(
...
def _build_binary_prompt(question: str, correct_answer: str, max_marks: int) -> str:
    """MCQ / True-False / Fill-in: binary correct/wrong."""
...
def build_evaluation_prompt(
    ...
    """
    Route to the correct prompt builder based on subject + question type.
    Returns (prompt_string, eval_mode).
    """
    q_type = question_type.lower().replace(" ", "_")
    subject_lower = subject.lower()

    # Binary question types — always binary regardless of subject
    ...
    # Auto-generate rubric if not provided (shouldn't happen in prod — log warning)
    ...
    # Default: CRITERIA (science, history, geography, etc.)
    return _build_criteria_prompt(question, correct_answer, rubric, marks, subject), mode


# ─────────────────────────────────────────────────────────────────────────────
# Response normalizer — make LLaVA output safe
# ─────────────────────────────────────────────────────────────────────────────
def normalize_response(raw: dict, max_marks: int) -> dict:
    """
    Cleans and validates the LLM response:
    - Ensures total_score <= max_marks
    - Recomputes total from criteria if mismatch
    - Clamps all values to valid range
    - Adds a low_confidence flag for teacher review
    """
    criteria = raw.get("criteria_scores", [])

    # Recompute total from criteria (don't trust LLM's sum)
    ...
    # Round to nearest 0.5 for holistic, integer for others
    ...
    return {
        ...
        "low_confidence": confidence < 0.5,  # Flag for teacher review
...
# LLaVA caller
# ─────────────────────────────────────────────────────────────────────────────
async def _call_llava(image_bytes: bytes, prompt: str) -> dict:
    ...
    payload = {
        ...
        "options": {
            ...
            "repeat_penalty": 1.0,
            "seed": 42,
            "num_predict": 1500,  # Enough for full criteria JSON
        },
        "messages": [
            {
                "role": "user",
    ...
    if not match:
        raise ValueError(f"No JSON found in LLaVA response: {content[:200]}")
...
async def evaluate_handwriting(
    ...
    try:
        raw = await _call_llava(image_bytes, prompt)
        ...
        return _error_response(max_marks, f"AI returned invalid JSON: {e}")
