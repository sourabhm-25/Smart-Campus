...
export default function Task() {
  ...
  const handleAddQuestion = (type) => {
    setQuestions(prev => {
      ...
      updated[type] = [...updated[type]];
      if (type === 'mcq') {
        updated[type].push({ question: "", options: { "A": "", "B": "", "C": "", "D": "" }, answer: "", isEditing: true });
      } else {
        updated[type].push({ question: "", answer: "", isEditing: true });
      }
      return updated;
    });
  };

  const handleToggleEditQuestion = (type, index, isEditing) => {
    setQuestions(prev => {
      if (!prev) return prev;
      const updated = { ...prev };
      updated[type] = [...updated[type]];
      updated[type][index] = { ...updated[type][index], isEditing };
      return updated;
    });
  };
  ...
  const handleAddFlashcard = () => {
    setFlashcards(prev => {
      ...
      updated.push({ front: "", back: "", hint: "", isEditing: true });
      return updated;
    });
  };

  const handleToggleEditFlashcard = (index, isEditing) => {
    setFlashcards(prev => {
      if (!prev) return prev;
      const updated = [...prev];
                    ...
                    <div key={i} className="q-card blue">
                      <div className="q-header-row">
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          {q.isEditing ? (
                            <button type="button" style={{fontSize: 12, fontWeight: 600, color: '#059669', background: '#d1fae5', padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer'}} onClick={() => handleToggleEditQuestion('short_answer', i, false)}>Save</button>
                          ) : (
                            <button type="button" className="btn-icon" onClick={() => handleToggleEditQuestion('short_answer', i, true)} title="Edit Question">
                      ...
                      <div className="q-question" style={{ display: 'flex', gap: '6px' }}>
                        ...
                        {q.isEditing ? (
                          ...
                          <div style={{ padding: '4px 6px', margin: '-4px -6px', whiteSpace: 'pre-wrap', flex: 1 }}>{q.question}</div>
                        )}
                      </div>
                      <div className="q-answer" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        Answer: 
                        {q.isEditing ? (
                          <input type="text" className="inline-input" style={{flex: 1, color: '#059669', fontWeight: 600}} value={q.answer} onChange={e => handleUpdateQuestion('short_answer', i, 'answer', e.target.value)} placeholder="Enter answer..." />
                        ) : (
                          <div style={{flex: 1, color: '#059669', fontWeight: 600, padding: '4px 6px', margin: '-4px -6px'}}>{q.answer}</div>
                        )}
                      </div>
                    </div>
                  ))}
                  ...
                  {questions.mcq.map((q, i) => (
                    <div key={i} className="q-card orange">
                      <div className="q-header-row">
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          {q.isEditing ? (
                            <button type="button" style={{fontSize: 12, fontWeight: 600, color: '#059669', background: '#d1fae5', padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer'}} onClick={() => handleToggleEditQuestion('mcq', i, false)}>Save</button>
                          ) : (
                            <button type="button" className="btn-icon" onClick={() => handleToggleEditQuestion('mcq', i, true)} title="Edit Question">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </button>
                          )}
                        ...
                        {Object.entries(q.options).map(([k, v]) => (
                          <div key={k} className="q-option" style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            ...
                            {q.isEditing ? (
                              ...
                              <div style={{flex: 1, padding: '4px 6px', margin: '-4px -6px'}}>{v}</div>
                            )}
                          </div>
                        ))}
                      </div>
                      <div className="q-answer" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        Answer: 
                        {q.isEditing ? (
                          <input type="text" className="inline-input" style={{width: '60px', color: '#059669', fontWeight: 600}} value={q.answer} onChange={e => handleUpdateQuestion('mcq', i, 'answer', e.target.value)} placeholder="e.g. A" />
                        ) : (
                          <div style={{width: '60px', color: '#059669', fontWeight: 600, padding: '4px 6px', margin: '-4px -6px'}}>{q.answer}</div>
                        )}
                      </div>
                    </div>
                  ))}
                  ...
                  {questions.fill_in_the_blanks.map((q, i) => (
                    <div key={i} className="q-card teal">
                      <div className="q-header-row">
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          {q.isEditing ? (
                            <button type="button" style={{fontSize: 12, fontWeight: 600, color: '#059669', background: '#d1fae5', padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer'}} onClick={() => handleToggleEditQuestion('fill_in_the_blanks', i, false)}>Save</button>
                          ) : (
                            <button type="button" className="btn-icon" onClick={() => handleToggleEditQuestion('fill_in_the_blanks', i, true)} title="Edit Question">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </button>
                          )}
                      ...
                      <div className="q-answer" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        Answer: 
                        {q.isEditing ? (
                          <input type="text" className="inline-input" style={{flex: 1, color: '#059669', fontWeight: 600}} value={q.answer} onChange={e => handleUpdateQuestion('fill_in_the_blanks', i, 'answer', e.target.value)} placeholder="Enter answer..." />
                        ) : (
                          <div style={{flex: 1, color: '#059669', fontWeight: 600, padding: '4px 6px', margin: '-4px -6px'}}>{q.answer}</div>
                        )}
                      </div>
                  ...
                  {questions.true_false.map((q, i) => (
                    <div key={i} className="q-card purple">
                      <div className="q-header-row">
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          {q.isEditing ? (
                            <button type="button" style={{fontSize: 12, fontWeight: 600, color: '#059669', background: '#d1fae5', padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer'}} onClick={() => handleToggleEditQuestion('true_false', i, false)}>Save</button>
                          ) : (
                            <button type="button" className="btn-icon" onClick={() => handleToggleEditQuestion('true_false', i, true)} title="Edit Question">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </button>
                          )}
                      ...
                      <div className="q-answer" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        Answer: 
                        {q.isEditing ? (
                          <input type="text" className="inline-input" style={{flex: 1, color: '#059669', fontWeight: 600}} value={q.answer} onChange={e => handleUpdateQuestion('true_false', i, 'answer', e.target.value)} placeholder="True or False..." />
                        ) : (
                          <div style={{flex: 1, color: '#059669', fontWeight: 600, padding: '4px 6px', margin: '-4px -6px'}}>{q.answer}</div>
                        )}
                      </div>
                  ...
                  {flashcards.map((f, i) => (
                    <div key={i} className="q-card" style={{ background: '#fff', borderColor: '#f0abfc' }}>
                      <div className="q-header-row">
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                          {f.isEditing ? (
                            <button type="button" style={{fontSize: 12, fontWeight: 600, color: '#059669', background: '#d1fae5', padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer'}} onClick={() => handleToggleEditFlashcard(i, false)}>Save</button>
                          ) : (
                            <button type="button" className="btn-icon" onClick={() => handleToggleEditFlashcard(i, true)} title="Edit Flashcard">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </button>
                          )}
                      ...
                      <div className="q-question" style={{ fontSize: '13px', color: '#9333ea', fontWeight: 'bold', display: 'flex', gap: '6px', alignItems: 'center' }}>
                        Hint: 
                        {f.isEditing ? (
                          <input type="text" className="inline-input" style={{flex: 1, color: '#9333ea'}} value={f.hint} onChange={e => handleUpdateFlashcard(i, 'hint', e.target.value)} placeholder="Hint..." />
                        ) : (
                          <div style={{flex: 1, color: '#9333ea', padding: '4px 6px', margin: '-4px -6px'}}>{f.hint}</div>
                        )}
                      </div>
                      <div className="q-question" style={{ display: 'flex', gap: '6px' }}>
                        <span style={{marginTop: '5px'}}>{i + 1}.</span>
                        {f.isEditing ? (
                          ...
                          <div style={{ padding: '4px 6px', margin: '-4px -6px', whiteSpace: 'pre-wrap', flex: 1 }}>{f.front}</div>
                        )}
                      </div>
                      <div className="q-answer" style={{ display: 'flex', gap: '6px' }}>
                        Back: 
                        {f.isEditing ? (
                          <textarea className="inline-input" rows={2} style={{flex: 1, color: '#059669', fontWeight: 600}} value={f.back} onChange={e => handleUpdateFlashcard(i, 'back', e.target.value)} placeholder="Back of flashcard..." />
                        ) : (
                          <div style={{flex: 1, color: '#059669', fontWeight: 600, padding: '4px 6px', margin: '-4px -6px', whiteSpace: 'pre-wrap'}}>{f.back}</div>
                        )}
                      </div>
                    </div>
                  ))}
